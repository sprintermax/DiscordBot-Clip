...
module.exports.run = async (client, message, args, database) => {
	...
	if (args.length < 1){
		...
		if (isNaN(numbers)) {
			message.channel.send(`${message.author} Isso não é um número! Escolha um número válido de lados.`).then(msg => msg.delete({ timeout: 10000 }));
		} else {
			if (numbers <= 1) {
				message.channel.send(`${message.author} você precisa escolher um número maior que 1`).then(msg => msg.delete({ timeout: 10000 }));
			} else {
