...
module.exports.run = async (client, message, args, database) => {
	...
	if (args[0]) {
		if (args[0].match(/(?<=<a?:.*:)\d*(?=>)/)) {
			...
			emoji = message.guild.emojis.cache.get(emojiid[0]) || client.emojis.cache.get(emojiid[0]);
		} else {
			emoji = message.guild.emojis.cache.get(args[0]) || client.emojis,cache.get(args[0]) || client.emojis.find(emoji => emoji.name == args[0]);
		}
		if (emoji) {
			if (emoji.animated) {
			...
			message.channel.send(`${message.author}\nNão encontrei o emoji, verifique se você especificou um emoji válido!\n(Emojis padrões do Discord ou de outros servidores que eu não estou não são válidos)`).then(msg => msg.delete({ timeout: 10000 }));
		}
	} else {
		message.channel.send(`${message.author}\nVocê precisa especificar o emoji!`).then(msg => msg.delete({ timeout: 10000 }));
	}
}